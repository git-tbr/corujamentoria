...
const fetchEbook = async (courseToken: string): Promise<void> => {
    loading.value = true;
    try {
        const res = await api.get(`/v1/course-ebook/${courseToken}/1`);
        const data = res.data;

        if (data.code != 1) throw new Error(data.message);

        ebook.value = data.ebook || '';
        if (ebook.value) {
            loading.value = false;
            await nextTick();
...
const renderPage = async (pageNumber: number): Promise<void> => {
    ...
    if (containerWidth === 0) {
        console.warn("Contêiner ainda tem largura 0. Pode haver um problema de CSS ou tempo de renderização.");
            ...
            <section class="container py-3">
                <div class="row m-0 mb-3 align-items-center bg-light p-3 rounded shadow-sm" v-reveal="'bottom'">
                    ...
                    <div class="col d-flex align-items-center justify-content-center gap-2">
                        ...
                        <input type="number" class="border" min="1" :max="totalPages" v-model="actualPage"
                            @keyup="searchPage" name="pageNumber" id="pageNumber">
                ...
                <div v-else-if="ebook.length > 0" class="row">
                    <div ref="pdfField" class="col-12" id="pdfField">
                        <div ref="pdfContainer" class="w-100" id="pdfContainer">
                        </div>
                    </div>
                </div>
