...
const simuladosFiltrados = computed(() => {
    ...
    if (search.value) {
        lista = lista.filter(s =>
            s.cat_title.toLowerCase().includes(search.value.toLowerCase())
        );
    }
...
const irParaSimulado = async (key: string, isSample: number) => {
    if (isSample == 1) {
        const confirmou = await showAlert({
            title: "Atenção!",
            message: "Para acessar este Banco de questões você precisa adquirir o curso primeiro. Deseja adquirir agora?",
            type: "warning",
            isConfirm: true
        });
...
onMounted(() => {
    ...
    if (token) fetchSimulados(token, userId);
});
</script>
                    ...
                    <div v-for="item in simuladosFiltrados" :key="item.cat_id" class="col-md-6 col-lg-3">
                        <div class="card h-100 border-0 rounded-4 shadow hover-up ">
                            ...
                            <div class="card-footer bg-transparent border-0 p-4 pt-0">
                                <button @click="irParaSimulado(item.cat_key, item.isSample)"
