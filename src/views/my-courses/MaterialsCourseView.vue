...
const fetchConteudoModulo = async (courseT: string, moduleT: number) => {
  loading.value = true
  try {
    const res = await api.get(`/api/purchased-courses/ead/${courseT}/module/${moduleT}`)
    if (res.data.code === 1) {
    ...
    if (selectedMaterialUrl.value) {
      loadPdfDocument(selectedMaterialUrl.value)
    }
  }
}
...
const loadPdfDocument = async (url: string): Promise<void> => {
  const name = selectedMaterialName.value
  const path = url
  ...
  if (nameEl) {
    nameEl.innerHTML = 'Carregando documento...'
  }
...
const returnToModule = () => {
  router.push({
    name: 'modulo',
    params: { moduleToken: moduleHash.value, courseToken: courseHash.value },
  })
}
            ...
            >
              <div class="card-body d-flex align-items-center">
                ...
                <div>
                  ...
                  <small>{{
                    selectedMaterialUrl === material.a_path
                      ? 'Aberto agora'
