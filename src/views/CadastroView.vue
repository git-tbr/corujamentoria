...
const showTinDescription = async () => {
  await showAlert({
    title: 'TIN',
    message:
      'Taxpayer Identification Number - Documento de identificação tributária. Alguns exemplos: NIF, SSN, CUIT, RFC.',
    type: 'success',
  })
}
...
const subscribeSubmit = async () => {
  if (emailCheckedClass.value !== '' || passCheckedClass.value !== '') {
    await showAlert({
